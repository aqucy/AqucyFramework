 ...
 */
public class Adidas_pingfenitemController extends Controller {
    @Before(Tx.class)
    public  void index()  throws Exception{
        Adidas_pingfenitemModel model = getModel(Adidas_pingfenitemModel.class,"adidas_pingfenitem");
        ...
        if ("add".equalsIgnoreCase(op)) {
            ...
            try {
                //查询老用户
                Adidas_pingfenitemModel um = Adidas_pingfenitemModel.dao.findById(model.getLong("id"));
                if(um==null){
                    jo.put("errcode", 1112);
            ...
            sb.append("FROM adidas_pingfenitem");
            model.removeNullValueAttrs();
            String[] keys = model.getAttrNames();
            Page<Adidas_pingfenitemModel>  page = null;
            if(keys.length>0){
                sb.append(" where 1=1");
                for (String key : keys) {
                ...
                page = Adidas_pingfenitemModel.dao.paginate((start/limit)+1,limit,"SELECT *",sb.toString(),model.getAttrValues());


            }else{
                page = Adidas_pingfenitemModel.dao.paginate((start/limit)+1,limit,"SELECT *",sb.toString());
            }
            ...
            if(StrKit.notBlank(ids)){
                Db.update("delete from adidas_pingfenitem where id in(" + ids + ")");
            }
        }else {
            render("/WEB-INF/view/project/adidas_pingfenitem.html");
        }
    }
}
